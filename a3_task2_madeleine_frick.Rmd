...
sw_split_name <- starwars %>% # start with starwars
  separate(col = name, # separate the names
           into = c("name_1","name_2"), # two columns 'name_1' & 'name_2'
...
sw_skin_sep <- starwars %>% # start from starwars data
  separate(col = skin_color, # separate the `skin color` column
           into = c("skin_1","skin_2"), # two columns
           sep = ",", # separate at the first *comma*
           extra = "merge", # merge all new spaces into second column
           remove = FALSE) # dont remove the original column
...
important_birthdays <- c(10161998, 12211964, 02102000, 12121994)
important_birthdays_dates <- mdy(important_birthdays)
important_birthdays_dates

class(important_birthdays_dates)
...
gasol_month_shots <- gasol_hookshots %>% 
  count(month, result) %>% 
  group_by(month) %>% 
...
storms_danger <- storms %>% 
  mutate(danger = case_when(
    status == "hurricane" ~ "high", #if it says hurricane in the status then put high
    TRUE ~ "moderate" #for all of the other things then the danger will say moderate
  ))
